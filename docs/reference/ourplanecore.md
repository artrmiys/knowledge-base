...
??? abstract "🆕 Что нового — история обновлений (нажми, чтобы раскрыть)"
    ...
    - **Count Similar — авто-счёт одинаковых символов.** Рядом с `Count` —
      кнопка `Similar`: обводишь рамкой **один** символ (окно, дверь, розетку,
      hardware), программа находит все похожие на листе **офлайн** (без
      интернета) и предлагает их как count-метки. Ползунок **порога** + живое
      синее превью; опц. галка — **перепроверка через AI**. Метки падают в
      активный `Count`-item или в новый `Similar Count`.
      :material-progress-wrench: *v1: ищет на **текущем** листе, по чёрно-белому
      контуру символа.*
    - **Multiline — линия с авто-офсетами.** При создании `Line`-item можно
      задать до **2 параллельных линий-компаньонов** (каждая — свой item с
      именем/цветом): рисуешь основную линию, программа сама строит смещённые
      параллели (напр. наружная/внутренняя грань стены, бровка дороги).
      *Офсеты генерируются в момент рисования.*
    - **Новые символы `Count`** — добавлены **star / triangle / diamond / ring**
      (к circle / cross / square). Дефолтный символ запоминается.
    - **Стропила (Rafters) на 3D-крыше** (таб `7 3D`). Группа `Rafters`:
      `Pick Faces` — тыкаешь конкретные скаты в 3D-вьюере (выбранные
      подсвечиваются), `Whole Roof` — вся крыша целиком; шаг `12/16/19.2/24`
      o.c. и сечение `2x6…2x12`. Считает **count + суммарную длину**
      (с поправкой на уклон), стены подрезаются под низ стропил.
      :material-progress-wrench: *Пока display-only, без выгрузки в takeoff.*
    - **Надёжность данных (релиз v2.0.0).** Атомарная запись `Data.xml` (файл не
      бьётся при сбое), **переносимый job** — пути внутри стали относительными,
      можно копировать папку job куда угодно без потери связей; AI-запросы с **3
      повторами**, **видимые ошибки** AI (раньше глохли молча), авто-архив
      старых AI-запросов и ротация логов.
    - **Export PDF с деревом папок.** В окне `Export` — дерево листов с
      подсветкой открытого листа + быстрый экспорт прямо из дерева.
    - **Repair при переносе job.** Скопировал/перенёс job в другую папку —
      `Repair Links` теперь сам находит листы по совпадающему **хвосту пути**, а
      не только по точному совпадению.
    - **Чёткий глубокий зум + быстрые переключения листов.** Текст остаётся
      резким **до зума 16×** (рендерится видимая область под текущий зум, а не
      растягивается картинка всего листа); соседние листы **греются заранее** в
      направлении листания, а размеры кэшей и число фоновых воркеров
      подстраиваются под **RAM/ядра** машины.
    - **Мелочи:** настраиваемый **шаг колеса зума** (`8 Settings → Defaults`);
      лента `Viewport` разбита, чтобы помещаться на ноутбучных экранах; исправлен
      «призрак» snap/trace на **повёрнутых листах** (`/Rotate ≠ 0`).
