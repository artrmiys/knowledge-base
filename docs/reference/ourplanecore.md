...
!!! abstract "Свежий цикл — UI/UX (что нового по исходнику)"
    - **F1 cheat-sheet overlay** — оверлей со всеми горячими клавишами по `F1`;
      на пустом старте (нет job) — **Start-карточка** с подсказками F1.
    - **Takeoff Templates dock** — PlanSwift-style панель шаблонов takeoff'ов на
      правой `Takeoffs`-полосе (зеркалит `Bookmarks`); `Auto Tree`/`From Pages` рядом.
    - **Transform popup** — `Flip` / `Rotate` / `Scale` свёрнуты в один popup на
      нижней полосе; нижняя полоса теперь **в два ряда**, добавлены `Tile M2` и
      `Current Excel` (перенесён на таб `Estimating`).
    - **Чище ribbon/табы (по дизайн-коду)** — компактная top tab+ribbon полоса,
      segmented main-pills, borderless ribbon-кнопки, иконки на табе `Page`
      (parity с `Main`), вторичные табы в OurCore-стиле (blue text + sage
      underline), Open/Import-меню перестроено, accent через тему.
    - **Старт/перф** — выбранная стартовая страница открывается напрямую; кэш
      первичных layer-рендеров (быстрее открытие тяжёлых PDF).

!!! abstract "Свежая неделя — производительность и снап (01–05.06.2026)"
    Это **поверх** UI-цикла выше: ~120 коммитов за неделю, главное —
    скорость, raster-листы и снап контура.

    - **Релиз v2 + навигация takeoff-дерева.** Перетаскивание takeoff'а
      больше **не открывает случайный лист** (был баг с `a502`): reveal
      разворачивает связанную строку Pages, но не выбирает её. Перемещение
      `section`/`count`-строки по-прежнему **прыгает на свой лист замера**
      (by design), а перенос целого takeoff'а **держит текущую страницу**.
      Добавлены **draggable page tabs**: перетащить вкладку между другими —
      переупорядочить, вытащить в пустое место — **отстегнуть в отдельное
      окно листа**. См. [Прочее новое](#new-misc).
    - **Viewport v4 — «молниеносно».** Одинаковые render-запросы
      coalesce (не дублируют PyMuPDF-рендер), большие **RAM-кэши** (layer
      bitmap, detail-тайлы, preview), **clipped detail-тайлы** для чёткого
      текста на **зуме 250–350 %+** (вместо мыла от upscale целого листа),
      фоновый **prefetch** соседних листов, **culling** лейблов/joist/overlay
      на быстром pan/zoom, **индексированная** синхронизация Pages↔Takeoffs
      (без обхода всего дерева на клик).
    - **Raster-листы (scanned / image-PDF).** Cache v5 + читаемые raster-листы:
      ...
      overviews для быстрого browsing, листы `A9*` классифицируются как
      **details**, idle-refresh кэша. Раньше image-импорт был мыльным/медленным.
    - **PDF exterior contour snap (Area + PDF Snap).** `Tab` циклит контуры:
      приоритет **внешнему контуру здания**, мостит мелкие разрывы через
      окна/проёмы, отсекает **внутренние двери** (узкие парные линии + дуги
      раскрытия). :material-progress-wrench: **В доработке** — на реальных
      листах результат пока не финальный, продолжается итерация.
    - **Joist summary labels.** Сводные joist-лейблы **видны по умолчанию** и
      независимы (не теряются при плотной раскладке).
    - **Blank job / Blank sheet.** Job **без PDF** и пустые листы
      (по умолчанию landscape `36" × 24"`): `Open / Import → Blank Job…`,
      контекст-меню Pages / палитра → `Blank Sheet`. Пустой лист пишет
      обычные `Data.xml` / `source.json` — viewport, scale, замеры и экспорт
      работают как на обычном листе.
...
| `Transform ▾` (popup) | — | `Flip H/V` / `Rotate` / `Scale` / сброс `0`/`1x` собраны в один popup |
...
| `Templates` (dock) | PlanSwift-style панель шаблонов takeoff'ов (зеркалит `Bookmarks`) |
...
## Bookmarks · Templates · docks { .kb-section-title .kb-st--green }

Две докуемые панели ускоряют работу: **Bookmarks** (куда смотреть) и
**Takeoff Templates** (что считать). Это **не** crop-bookmarks для AI — те живут
в AI Manager.

### Page Bookmarks (док под Pages)

Сохранённый **вид листа** — страница + текущий zoom/позиция. Нужен, чтобы за один
клик возвращаться к нужному месту чертежа (узел, schedule, деталь).

| Кнопка | Действие |
| --- | --- |
| `Add` (++b++ затем ++k++) | Сохранить текущую страницу + zoom как закладку |
| `Open` (или двойной клик) | Открыть сохранённый вид |
| `Rename` / `Delete` | Переименовать / удалить |

Список — колонки `Name` / `Page` / `View`. Включается тумблером
**«Show Bookmarks below Pages»** (отдельная панель под деревом Pages).

### Takeoff Templates (док под Takeoffs)

Переиспользуемое **дерево takeoff-папок/итемов** (PlanSwift-style). Собрал типовую
структуру (например, COM-набор секций) один раз — и разворачиваешь её на каждом
новом job, не пересобирая вручную.

| Кнопка | Действие |
| --- | --- |
| `Save Current` | Сохранить текущее дерево Takeoffs как шаблон |
| `Apply` (или двойной клик) | Создать выбранный шаблон под текущей Takeoffs-папкой |
| `Rename` / `Delete` | Переименовать / удалить |

Док зеркалит Bookmarks, живёт под табами Takeoffs (можно отстегнуть в нижний ряд).
Пусто → подсказка «Build a Takeoffs tree, then Save Current».
...
!!! tip "Bookmark vs Template vs Crop-bookmark — не путать"
    ...
    - **Crop bookmark** = фрагмент чертежа в очередь AI (`Run New` / `Retry Failed`),
      см. [Таб 6 AI Manager](#tab-6-ai-manager).

### Новые UI-удобства

- **`F1` — cheat-sheet overlay:** всплывает оверлей со всеми горячими клавишами
  (tools / viewport / trees / 3D). Не нужно помнить — нажал `F1`, посмотрел.
- **Start-карточка (пустой старт):** когда job ещё не открыт, в центре —
  карточка «с чего начать» (`Open / Import`) с подсказками `F1`, а не пустой экран.
- **Transform popup:** `Flip H/V`, `Rotate`, `Scale` и сбросы `0`/`1x` собраны в
  один popup на нижней полосе — tool-strip стал чище и компактнее (два ряда).
...
| `PDF Snap` + `Area` → `Tab` | Трассировка **контура**: приоритет внешнему контуру здания, мостит окна/проёмы, отсекает внутренние двери *(в доработке)* | Area-инструмент на vector/raster-листе |
...
- **Draggable page tabs** — вкладки открытых листов перетаскиваются: между другими вкладками — переупорядочить, в пустое место — отстегнуть лист в отдельное окно.
