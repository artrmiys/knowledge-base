...
??? abstract "🆕 Что нового — история обновлений (нажми, чтобы раскрыть)"
    ...
    Косметический и UX-проход по программе.

    - **Иконки папок** в деревьях (Pages и Takeoffs) — тонкая контурная папка в
      цвете темы, меньше прежнего. Папка **закрашена**, если внутри есть листы /
      takeoff'ы (в любой вложенности), и **пустой контур**, если пусто или внутри
      только пустые подпапки — видно с одного взгляда, где есть данные.
    - **Выделение строк в `Bookmarks` и `AI Inbox`** — раньше в тёмной теме строка
      подсвечивалась бледно-белым и текст было плохо видно; теперь — чёткая
      сплошная заливка как в деревьях, не тускнеет при переходе фокуса. Заголовки
      колонок `AI Inbox` тоже подсвечиваются аккуратно.
    - **Вкладка `Annotation`** переехала **сразу после `Page`** и переоформлена:
      инструменты (`Highlight` / `Draw` / `Arrow` / `Box` / `Cloud` / `Area` /
      `Note`) теперь с иконками, в два ряда, без рамок. Толщина линии и цвет —
      прямо в ленте: **ползунок `Width`** + палитра цветов (больше не нужно лезть
      в меню `Style`).
    - **`Ruler`** вернулся в **нижнюю панель** инструментов (рядом с
      `Pan`/`Select`/`Scale`) обычной кнопкой.
    - **Панель `Templates`** (справа) — лишние кнопки убраны в сворачиваемый блок
      **`Edit`**; на виду остались выбор шаблона и `Create`.
