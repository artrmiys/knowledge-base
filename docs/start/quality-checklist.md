...
1. **Секции** — ожидалась, но отсутствует (есть у ≥60% похожих проектов).
2. **Строки** — нет типовой строки внутри присутствующей секции (≥70% peers).
3. **Количества** — значение вне типового диапазона `(секция, строка)`.
