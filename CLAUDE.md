# CLAUDE.md — Operating Manual for E-Wood Knowledge Base
...
**E-Wood Knowledge Base** — рабочая wiki по estimating (takeoff, COM/EWP,
...
- **Прод-URL:** https://artrmiys.github.io/knowledge-base-e-wood/
- **Репо:** https://github.com/artrmiys/knowledge-base-e-wood
...
Все команды запускать **из корня проекта** (`E:\---\Work\knowledge-base-e-wood`).
...
⚠️ Локальный URL — **`http://127.0.0.1:8000/knowledge-base-e-wood/`**, а не просто `/`.
Это потому что в `mkdocs.yml` задан `site_url` под GitHub Pages project page —
serve уважает его и поднимает контент по тому же префиксу. Если запросить `/`
без префикса, будет 302 → редирект на правильный URL.
...
- [ ] Локальный preview — `http://127.0.0.1:8000/knowledge-base-e-wood/` — проверить визуально.
...
1. **`site_url` влияет на `mkdocs serve`.** Локальный preview живёт на
   `/knowledge-base-e-wood/` префиксе, а не на `/`. Это правильно — совпадает
   с проды́. Просто помни.
